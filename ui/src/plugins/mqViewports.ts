 ...
 */
function convertBreakpointsToMediaQueries(breakpoints: Record<string, json2mq.QueryObject>): Record<string, string> {
  const keys = Object.keys(breakpoints);
  const values = keys.map((key) => breakpoints[key]);
  const media: Record<string, string> = values.reduce((queries: Record<string, string>, value, index) => {
    let builtQuery = '';
...
const breaks = ref(
  Object.keys(DEFAULT_BREAKPOINTS).reduce((final, key) => {
...
export default {
  install: (
    app: App<Element>,
    { breakpoints = DEFAULT_BREAKPOINTS, defaultBreakpoint = 'lg', $window = window } = {} as VueMqTopDownConfig
  ) => {
    let hasSetupListeners = false;
    currentBreakpoint.value = defaultBreakpoint;
    breaks.value = Object.keys(DEFAULT_BREAKPOINTS).reduce((final, key) => {
      return Object.assign(final, {
        [key]: key === 'lg',
      });
...
export function useMqViewports() {
  return {
    $mq: computed(() => {
      return currentBreakpoint.value;
    }),
